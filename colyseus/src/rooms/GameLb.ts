...
// Note to self => Atomically running LUA script
const SCRIPT = `
...
export function pointsFor(level: number, minutes: number): number {
    ...
    return Math.round(
    Math.max(base * FLOOR, base - LOSS_PER_MINUTE * Math.max(0, minutes)),
    );
}
...
export async function getAllTeamScores(): Promise<{ rank: number; teamId: string; points: number }[]>{
